            ...
            par(mar=c(6,7,5,4))
            N = 15
            colors = c(20, 21, 22, 24, 26, 27) #terrain.colors(N)
            models = c(12, 1:5)
            n = length(models)
            names  = c("EMSEMBLE", "M1", "M2", "M3", "M4", "M5" )
            plot( lowess(t[,12], f=1/10), t='b', pch=20, col=20, lwd=4, xlab="FOLD NUMBER", ylim=c(0.35,0.45), main="LOWESS[ LOG-LOSS( CV FOLD[i] ) ]", ylab="LOG LOSS FOR FOLD", cex=2.0, cex.axis=3.0, cex.lab=3.0, cex.main=3.0 )
            grid(lwd=2)
            fval=1/8
            for( i in 2:n ) { l=i-1; points( lowess(t[,l], f=fval), t='b', pch=20+l, col=colors[l+1], cex=1.0, lwd=1+l/5 ) }
            legend("bottomright", legend = names, col = c(20,colors[2:n]), pch=c(20:25), ncol = 3, cex = 3.0, lwd = 3)

            # models = c(12,1:9)
            # plot( lowess(t[,12], f=1/10), t='b', pch='o', col=20, lwd=4, xlab="FOLD NUMBER", ylim=c(0.35,0.45), main="LOWESS[ LOG-LOSS( CV FOLD[i] ) ]", ylab="LOG LOSS FOR FOLD", cex=1.2, cex.axis=2.0, cex.lab=2.0, cex.main=2.0 )
            # fval=1/8
            # n = length(models)
            # for( i in 2:n ) { l=models[i]; points( lowess(t[,l], f=fval), t='b', pch=10+i, col=colors[l], cex=0.8, lwd=1+l/5 ) }
            ...
            t = t(PROB_METRICS_MATRIX[,1:iter])
            models = c(1:9, 12)
            boxplot( cor(t[,models]), cex=2.0, cex.axis=2.0, cex.main=2.0, cex.lab=2, col=terrain.colors(15), horizontal=T, names=model_names, main="Boxplot of Correlation of Log-Loss Performance\nAcross Folds Per-Model", las=2 ) 
            dev.off()
            # ############################################################################################
            ...
            t = t(PROB_METRICS_MATRIX[,1:iter])
            models = c(1:9, 12)
            boxplot( t[,models], cex=2.0, cex.axis=2.0, cex.main=2.0, cex.lab=2, col=terrain.colors(15), horizontal=T, ylim=c(0.25,0.55), names=model_names, main="Boxplot of Log-Loss Performance\nAcross Folds Per-Model", las=2 ) 
            dev.off()
            # ############################################################################################
