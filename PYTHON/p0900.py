class RLEIterator:
    ...
    def next(self, n: int) -> int:
        # exhausted the list
        if self.ix > self.size: return StopIteration

        v = -1
        while self.ix < self.size:
            c, v = self.items[self.ix]
            if c == 0:
                self.ix += 1
            elif c >= n:
                self.items[self.ix] = [c-n, v]
                return v
            else:
                self.items[self.ix] = [0, v]
                self.ix += 1
                n -= c

        if self.ix >= self.ix: # exhausted the list
            return -1
