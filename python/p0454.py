class Solution:
    ...
    many tuples (i, j, k, l) there are such that A[i] + B[j] + C[k] + D[l] is zero.

    To make problem a bit easier, all A, B, C, D have same length
    of N where 0 ≤ N ≤ 500. All integers are in the range of -228
    ...
    def fourSumCount(self, A: List[int], B: List[int], C: List[int], D: List[int]) -> int:
        ...
        for i in range(n):
            for j in range(n):
                v1 = A[i] + B[j]
                v2 = C[i] + D[j]
                if v1 not in f1: f1[v1] = 0
                if v2 not in f2: f2[v2] = 0
                f1[v1] += 1
                f2[v2] += 1
