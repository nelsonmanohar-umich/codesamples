...
# #############################################################################
def place_remote_request_for(xvar=None, xlevel=None, opmode=OPMODE_TRAIN, rdata="", cvfold=1, do_rje=True, delay=5.0):
    ...
    if do_rje:
        time.sleep(delay)
        msg = mqformatter(["OPMODE", "XVAR", "XLEVEL", "CVFOLD", "RDATA"], [opmode, xvar, xlevel, cvfold, rdata], uniq_id)
...
if __name__ == "__main__":
    ...
    args_d = dict(zip(['OPMODE', 'XVAR', 'XLEVEL', 'DRYRUN', 'RDATA', 'CVFOLD'],
                      [args.opmode, args.xvar, args.xlevel, args.dryrun, args.rdata, args.cvfold]))
    try:
        print >>sys.stderr, '-' * 80
        for x in args_d:
