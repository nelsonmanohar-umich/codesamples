...
# ######################################################################################################
readWithAutoClasses = function( csv, ... ) {
    t = read.table( csv, nrows=10 )
    dClasses = sapply(t[2,], function(x) class(x))
    t = read.table( csv, header=TRUE, colClasses=dClasses, ... )
    return ( t )
}
# ######################################################################################################
