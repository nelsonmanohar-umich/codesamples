...
RECODE_FACTOR = function( XYMAT, attr_name , N=5, other="other", new_attr_name="", approx=FALSE ) {
    ...
    if ( other %in% activated_levels ) {
        XYMAT[,new_attr_name] = factor( new_attr, rev(activated_levels), ordered=TRUE ) 
    } else {
...
# #####################################################################################
RECODE_FACTOR_USING = function( attr_name, other="other", activated_levels=c(), new_attr_name="" ) {
